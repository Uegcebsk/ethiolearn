...
if (isset($_GET['id']) && !empty($_GET['id'])) {
    ...
    $sql = "SELECT message.*, 
                   CASE WHEN message.sender_type = 'lecturer' THEN lectures.l_img 
                        WHEN message.sender_type = 'student' THEN students.stu_img 
                   END AS profile_pic,
                   CASE WHEN message.sender_type = 'lecturer' THEN lectures.l_name
                        WHEN message.sender_type = 'student' THEN students.stu_name
                   END AS sender_name,
                   CASE WHEN message.sender_type = 'lecturer' THEN lectures.online_status
                   END AS online_status_lecturer
            FROM message
...
<div class="container mt-5" style="padding:7%;">
    <div class="card mb-4">
        <div class="card-header bg-light">
            <div class="media">
                <a href="#" class="text-decoration-none position-relative">
                    <img src="<?php echo $main_message['profile_pic']; ?>" class="mr-3 rounded-circle" alt="Profile Pic" style="width: 40px; height: 40px; position: relative;">
                    <!-- Dot indicating online or offline status for lecturers -->
                    <div class="dot <?php echo ($main_message['online_status_lecturer'] === 'online') ? 'dot-online' : 'dot-offline'; ?>"></div>
                </a>
        ...
        <div class="card-body">
            <?php while ($message = $all_messages_result->fetch_assoc()) { ?>
                <div class="message <?php echo ($message['sender_type'] == $main_message['sender_type']) ? 'received' : 'sent'; ?>">
    ...
    <div class="card mb-4">
        <div class="card-body">
            <form action="<?php echo ($main_message['sender_type'] == 'lecturer') ? 'send_reply.php' : 'send_reply.php'; ?>" method="POST">
...
<style>
    ...
    .message.received {
        ...
        border-radius: 10px;
        padding: 8px;
        margin-left: 0; /* Align received messages to the left */
        margin-right: auto;
    }
    ...
    .message-content small {
        text-align: <?php echo ($main_message['sender_type'] == 'lecturer') ? 'left' : 'right'; ?>; /* Align timestamps based on sender type */
    }
...
<?php
        ...
        header("Location: inbox_student.php");
        exit();
    }
} else {
    // Redirect back to inbox page if message ID is not provided
    echo "<script>setTimeout(()=>{window.location.href='inbox_student.php';},300);</script>";
