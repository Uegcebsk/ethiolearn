...
if (isset($_REQUEST['reqUpdate'])) {
    if (($_REQUEST['course_name'] == "") || ($_REQUEST['course_desc'] == "") ||  ($_REQUEST['course_author'] == "")  ||  ($_REQUEST['course_duration'] == "")  ||  ($_REQUEST['course_price'] == "") ||  ($_REQUEST['course_lessons'] == "")) {
        ...
        $sql = "UPDATE course SET course_id='$cid',course_name='$cname',course_desc='$cdesc',lec_id='$cauthor',course_duration='$cduration',course_price='$cprice',course_lessons='$clessons' WHERE course_id='$cid'";
...
<div class="col-sm-6 mt-5 jumbotron">
    ...
    <select class="form-control" name="course_author" id="course_author">
        ...
        $result = $conn->query($sql);
        while ($rowLec = $result->fetch_assoc()) {
            ?>
            <option value="<?php echo $rowLec['l_id']; ?>" <?php if (isset($row['course_author']) && $row['course_author'] == $rowLec['l_id']) echo "selected"; ?>>
                <?php echo $rowLec['l_name']; ?>
            </option>
        <?php } ?>
    </select>
</div>
