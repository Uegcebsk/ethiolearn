...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if (empty($cpassword)) {
        ...
        $error_msg['C_password1'] = "Passwords do not match";
    }

    if (empty($error_msg)) {
        // Verify the token and expiry
        $stmt = $conn->prepare("SELECT reset_token, reset_expiry FROM students WHERE reset_token=?");
        ...
        if ($result->num_rows > 0) {
            ...
            echo "Token: " . htmlspecialchars($row['reset_token']) . "<br>";
            echo "Expiry: " . htmlspecialchars($row['reset_expiry']) . "<br>";
            echo "Current Time: " . $current_time->format('Y-m-d H:i:s') . "<br>";

            if ($expiry_time > $current_time) {
                $hashed_password = password_hash($password, PASSWORD_DEFAULT);
                $stmt = $conn->prepare("UPDATE students SET stu_pass=?, reset_token=NULL, reset_expiry=NULL WHERE reset_token=?");
